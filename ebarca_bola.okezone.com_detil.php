...
//echo $html;die;
//$konten
$ret = $html->find('div[class=bg-euro-body-content-right]',0);

//$penulis
$div_penulis = $ret->find('p[class=imgnews-jurnalist]',0);
$penulis = trim(mysql_escape_string($div_penulis->plaintext));

//$div_konten = $ret->find('div[id=detail]',0);
if($div_konten = $ret->find('div[class=bg-euro-body-news-hnews-content-textisi]',0)){
...
//$waktu
$div_waktu = $ret->find('p[class=imgnews-datetime]',0);
$waktu = trim(mysql_escape_string($div_waktu->plaintext));

//$img
$div_img = $ret->find('div[class=img-col-detail] img',0);
$img = trim(mysql_escape_string($div_img->src));

// img_tumb
if(!$img_tumb || !$img_tumb){
    die;
}
